...
def post_deliver_bottles(potions_delivered: list[PotionInventory], order_id: int):
    ...
    with db.engine.begin() as connection:
        for potion in potions_delivered:
            ...
            connection.execute(sqlalchemy.text("INSERT INTO account_ledger_entries (account_id, account_transaction_id, change, transaction_type) VALUES (:my_account_id, :transaction_id, :qty, 'potion'), (:my_account_id, :transaction_id, :red_ml, 'red_ml'), (:my_account_id, :transaction_id, :green_ml, 'green_ml'), (:my_account_id, :transaction_id, :blue_ml, 'blue_ml'), (:my_account_id, :transaction_id, :dark_ml, 'dark_ml')"), [{"my_account_id" : 1, "transaction_id" : transaction_id, "qty" : potion.quantity, "red_ml" : -(potion.potion_type[0] * potion.quantity), "green_ml" : -(potion.potion_type[1] * potion.quantity), "blue_ml" : -(potion.potion_type[2] * potion.quantity), "dark_ml" : -(potion.potion_type[3] * potion.quantity)}])
    ...
    with db.engine.begin() as connection:
        ...
        while loop or i < potion_limit // 6:
            ...
            for count, item in enumerate(item_info):
                test1 = item.qty <= i or item.sku == "RAINBOW_0"
                test2 = test1 and item.qty < 50
                if item.red_qty <= red_ml and item.green_qty <= green_ml and item.blue_qty <= blue_ml and item.dark_qty <= dark_ml and sum(item_list) + total_potions < potion_limit and test2:
                    item_list[count] += 1
                    red_ml -= item.red_qty
